...
fn get_union(a: u64, b: u64) -> u64 {
 ...
 fn foo( now: u64, mut values: Vec<u64>, result: u64, counter: &mut u64) -> ()
 {
    if values.len() == 0 {
        //println!("res: {}", now);
        if now == result {
    ...
    foo( now * val, values.clone(), result, counter);

    foo( get_union(now, val), values.clone(), result, counter);
...
fn part1() -> u64 {
    ...
    for line in input.lines() {
        ...
        foo( val, values, result, &mut counter);
